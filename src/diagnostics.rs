...
pub struct WindowingReport {
    ...
    pub gnome_shell_extension_dbus: Check,
...
fn windowing_report() -> WindowingReport {
    ...
    let gnome_shell_extension_dbus = gdbus_call_check(
        "dev.avifenesh.ComputerUseLinux.WindowControl",
        "/dev/avifenesh/ComputerUseLinux/WindowControl",
        "dev.avifenesh.ComputerUseLinux.WindowControl.ListWindows",
        &[],
    );
    let can_list_windows = gnome_shell_introspect.ok || gnome_shell_extension_dbus.ok;
    ...
    .ok;
    let can_focus_windows = gnome_shell_extension_dbus.ok;
...
fn readiness_report(
    ...
    let recommended_next_step = if !can_build_accessibility_tree {
        ...
        "Run setup_window_targeting to install the Linux Computer Use GNOME Shell extension backend, or enable GNOME Shell window introspection before using targeted keyboard input.".to_string()
    } else if !can_focus_windows {
        "Run setup_window_targeting to install the Linux Computer Use GNOME Shell extension backend before using exact window_id, title, or terminal-targeted input.".to_string()
...
mod tests {
    ...
    fn windowing_report(can_list_windows: bool, can_focus_windows: bool) -> WindowingReport {
        WindowingReport {
            ...
            gnome_shell_extension_dbus: if can_focus_windows {
                Check::ok("ok")
            } else {
                Check::fail("missing")
            },
